...
public class StairwayLoggingHooks implements StairwayHook {
    ...
    public HookAction startFlight(FlightContext context) {
        ...
        logger.info(FlightLogFormat, FLIGHT_OPERATION_START, context.getFlightClassName(),
    ...
    public HookAction startStep(FlightContext context) {
        ...
        logger.info(StepLogFormat, FLIGHT_STEP_OPERATION_START, context.getFlightClassName(), context.getFlightId(),
    ...
    public HookAction endFlight(FlightContext context) {
        ...
        logger.info(FlightLogFormat, FLIGHT_OPERATION_END, context.getFlightClassName(),
            context.getFlightId(), Instant.now().atZone(ZoneId.of("Z")).format(DateTimeFormatter.ISO_INSTANT));
    ...
    public HookAction endStep(FlightContext context) {
        ...
        logger.info(StepLogFormat, FLIGHT_STEP_OPERATION_END, context.getFlightClassName(), context.getFlightId(),
            context.getStepClassName(), context.getStepIndex(), context.getDirection().name(),
            Instant.now().atZone(ZoneId.of("Z")).format(DateTimeFormatter.ISO_INSTANT));
        performanceLogger.timerEndAndLog(getStepTimerName(context.getFlightId()), context.getFlightId(),
            context.getFlightClassName(), FLIGHT_STEP_OPERATION_END, context.getStepIndex());
        clearMdcKeys();
        return HookAction.CONTINUE;
    }
